                                      ...
                                      <div className="number-grid">
                                          {cardOptions.map(opt => {
                                              ...
                                              return (
                                                  <button 
                                                    key={opt.id} 
                                                    className={`num-btn ${isTaken || isBeingViewed ?'taken':''} ${isMine?'mine':''}`} 
                                                    onClick={() => !isTaken && !isMine && !isBeingViewed && setSelectedOption(opt)} 
                                                    disabled={isTaken || isMine || isBeingViewed}
                                                  >
                                                      {opt.label || `${opt.id}`}
                                                      {isTaken && <span className="taken-badge">{t('locked')}</span>}
